...
# Creamos el Blueprint para el Login
...
def mostrar_login():
    ...
    if request.method == 'POST':
        ...
        # --- CREDENCIALES MAESTRAS DEL SISTEMA ---
        if usuario == 'SAMU' and password == '716285':
            # Iniciamos la sesión con privilegios máximos
            session['usuario_id'] = 1
            session['nombre'] = 'SAMU'
            session['rol'] = 'Admin' 
            
            # Lo enviamos al panel en modo autorizado
            return redirect(url_for('inicio.panel_principal', modo='auth'))
        else:
    ...
    <head>
        ...
        <style>
            ...
            .blob-blue {
                width: 60vw; height: 60vw; background: #0ea5e9; opacity: 0.25; 
                top: -10%; left: -10%; animation: movDinamicoAzul 18s infinite ease-in-out;
            }
            .blob-pink {
                width: 60vw; height: 60vw; background: #f9a8d4; opacity: 0.20; 
                bottom: -10%; right: -10%; animation: movDinamicoRosa 22s infinite ease-in-out;
            }
            ...
            .brand-group {
                display: flex; flex-direction: column; align-items: center;
                transform: translateY(12vh); transition: transform 1.2s cubic-bezier(0.25, 1, 0.5, 1);
            }
            .brand-group.move-up-group { transform: translateY(-3vh); }
            .logo-img {
                width: 240px; opacity: 0; transform: translateY(0);
            ...
            .brand-samu {
                font-size: 5rem; color: #0f172a; opacity: 0; margin-top: 5px;
                letter-spacing: 0.08em; transition: opacity 1.2s ease;
            }
            .brand-samu.fade-in { opacity: 1; }
            .action-area {
                width: 100%; opacity: 0; visibility: hidden; transform: translateY(20px);
            ...
            .btn-comenzar {
                background: #0f172a; color: #ffffff; padding: 16px 40px; border-radius: 16px;
                border: none; font-size: 1.2rem; letter-spacing: 2px;
                box-shadow: 0 10px 25px rgba(15, 23, 42, 0.15); transition: all 0.3s ease;
            ...
            .btn-panel {
                background-color: transparent; color: #64748b; border: 2px solid #cbd5e1;
                padding: 14px 40px; border-radius: 16px; font-size: 1rem; letter-spacing: 1.5px;
                width: 90%; transition: all 0.3s ease; text-transform: uppercase; margin-top: 5px;
            }
            .btn-panel:hover { background-color: #f1f5f9; color: #0f172a; border-color: #94a3b8; }
            
            /* FORMULARIO DE LOGIN */
            #login-box {
                width: 90%; margin: 0 auto; background: rgba(255, 255, 255, 0.95);
                backdrop-filter: blur(25px); -webkit-backdrop-filter: blur(25px);
                padding: 35px 30px; border-radius: 24px; box-shadow: 0 20px 50px rgba(0, 0, 0, 0.06);
            ...
            .form-control:focus { background-color: #ffffff; border: 1px solid #3b82f6; box-shadow: 0 0 0 4px rgba(59, 130, 246, 0.1); }
            .btn-ingresar { background: linear-gradient(90deg, #0ea5e9 0%, #3b82f6 100%); color: white; border-radius: 12px; padding: 15px; font-size: 1.1rem; letter-spacing: 2px; border: none; text-transform: uppercase; transition: filter 0.3s ease; margin-top: 15px; box-shadow: 0 8px 20px rgba(14, 165, 233, 0.2); width: 100%; }
            .btn-ingresar:hover { filter: brightness(1.1); }
            .alert-error { color: #dc2626; font-family: 'Arial', sans-serif; font-size: 0.9rem; text-align: center; margin-bottom: 15px; background: #fef2f2; padding: 10px; border-radius: 8px; border: 1px solid #fecaca; }
    ...
    <body>
        ...
        <div class="main-wrapper">
            ...
            <div class="action-area" id="actionArea">
                ...
                <div style="width: 100%; padding: 0 10px; display: flex; justify-content: center;">
                    <a href="/panel?modo=visual" style="text-decoration: none; width: 100%; display:flex; justify-content: center;">
                        <button type="button" class="btn-panel">
                            PANEL PRINCIPAL
                        </button>
                    </a>
                </div>
        ...
        <script>
            ...
            window.onload = function() {
                ...
                // Si hubo un error en el login, mostrar el cuadro automáticamente
                {% if error %}
                    animOut(document.getElementById('btnComenzarWrapper'), () => {
                        animIn(document.getElementById('loginWrapper'));
                    });
                {% endif %}
            };
            ...
            document.getElementById('btnComenzar').addEventListener('click', () => {
                animOut(btnWrap, () => { animIn(loginWrap); });
            });

            document.getElementById('btnVolver').addEventListener('click', (e) => {
                e.preventDefault();
                animOut(loginWrap, () => { animIn(btnWrap); });
            });
        </script>
    </body>
    </html>
