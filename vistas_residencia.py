...
# IMPORTACIONES MODULARES (Protegidas por try/except para evitar caídas si faltan)
try:
    from mod_01_jornal import JORNAL_HTML
except ImportError:
    JORNAL_HTML = "<div class='step-view active' id='step1'><p>Error cargando mod_01_jornal.py</p></div>"

try:
    from mod_02_personal import PERSONAL_HTML
except ImportError:
    PERSONAL_HTML = "<div class='step-view' id='step2'><p>Error cargando mod_02_personal.py</p></div>"
...
def redaccion_asiento_residente():
    ...
    <head>
        ...
        <style>
            ...
            .stepper-container {{ position: fixed; top: var(--nav-height); left: 0; width: 100%; background: rgba(255,255,255,0.85); backdrop-filter: blur(20px); border-bottom: 1px solid rgba(0,0,0,0.08); z-index: 900; padding: 12px 20px; overflow-x: auto; white-space: nowrap; display: flex; gap: 12px; opacity: 0; pointer-events: none; transition: opacity 0.5s; }}
            ...
            /* ==========================================
               CUADERNO FÍSICO CORREGIDO (ALINEACIÓN Y TAMAÑO)
               ========================================== */
            ...
            /* RAYADO Y TAMAÑO DE LETRA AUMENTADO */
    ...
    <body>
        ...
        <div class="split-layout" id="mainLayout">
            <div class="form-column">
                <form id="formResidencia" onsubmit="event.preventDefault();" oninput="sincronizarDatos()">
                    ...
                    <div class="step-view" id="step3"><div class="step-title">3.- Partidas</div><div class="alert alert-light border">Módulo 03 pendiente de ensamblar.</div><input type="hidden" class="req-step3" id="v_partidas"></div>
                    <div class="step-view" id="step4"><div class="step-title">4.- Mayor Metrado</div><textarea class="form-control req-step4" id="v_mayor_m" rows="5" oninput="sincronizarDatos()"></textarea></div>
                    <div class="step-view" id="step5"><div class="step-title">5.- Sub Partidas</div><textarea class="form-control req-step5" id="v_sub_p" rows="5" oninput="sincronizarDatos()"></textarea></div>
                    <div class="step-view" id="step6"><div class="step-title">6.- Actividades</div><textarea class="form-control req-step6" id="v_activ" rows="5" oninput="sincronizarDatos()"></textarea></div>
                    <div class="step-view" id="step7"><div class="step-title">7.- Almacén</div><textarea class="form-control req-step7" id="v_almacen" rows="5" oninput="sincronizarDatos()"></textarea></div>
                    <div class="step-view" id="step8"><div class="step-title">8.- Maquinaria</div><textarea class="form-control req-step8" id="v_maquina" rows="5" oninput="sincronizarDatos()"></textarea></div>
                    <div class="step-view" id="step9"><div class="step-title">9.- Herramientas</div><textarea class="form-control req-step9" id="v_herram" rows="4" oninput="sincronizarDatos()"></textarea></div>
                    <div class="step-view" id="step10"><div class="step-title text-danger">10.- Ocurrencias</div><textarea class="form-control border-danger req-step10" id="v_ocurrencia" rows="6" oninput="sincronizarDatos()"></textarea></div>
                </form>
            </div>
        ...
        <script>
            ...
            // RENDERIZADO EN VIVO CORREGIDO
            function sincronizarDatos() {{
                if(!g_numAsiento) return;
                let as_str = g_numAsiento.padStart(4, '0');
                
                // CABECERA ALINEADA (Tamaño aumentado)
                let textoPapel = `
                ...
                const vOper = (document.getElementById('v_oper').value || '0').padStart(2, '0'); const vOfic = (document.getElementById('v_ofic').value || '0').padStart(2, '0'); const vPeon = (document.getElementById('v_peon').value || '0').padStart(2, '0');
                const vMeca = (document.getElementById('v_meca').value || '0').padStart(2, '0'); const vCtrl = (document.getElementById('v_ctrl').value || '0').padStart(2, '0'); const vOpe = (document.getElementById('v_ope_maq').value || '0').padStart(2, '0');
