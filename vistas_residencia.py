...
def redaccion_asiento_residente():
    ...
    # Fecha por defecto para el input inicial (hoy)
    ...
    <body>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content" style="border-radius: 24px; border: none; box-shadow: 0 20px 50px rgba(0,0,0,0.2);">
                    ...
                    <div class="modal-body px-5 pb-4">
                        <p class="text-muted small text-center mb-4">Complete los datos legales para aperturar el folio de hoy en el cuaderno de obra digital.</p>
                        <div class="mb-3">
                        ...
                        <button type="button" class="btn btn-primary btn-lg w-100 rounded-pill fw-bold" onclick="iniciarAsiento()">Comenzar Redacción <i class="bi bi-arrow-right ms-2"></i></button>
                    </div>
                </div>
            </div>
        </div>

        <div class="stepper-container" id="stepperBar" style="opacity:0; pointer-events:none; transition: opacity 0.5s;">
        ...
        <script>
            ...
            function iniciarAsiento() {
                ...
                if(!g_numAsiento || !rawDate) { alert("Complete el número de asiento y la fecha para iniciar."); return; }
            ...
            function jumpToStep(stepIndex) {
                ...
                setTimeout(() => {
                    ...
                    if (currentStep > 1) { btnLobby.classList.add('d-none'); btnAtras.classList.remove('d-none'); } 
                    else { btnAtras.classList.add('d-none'); btnLobby.classList.remove('d-none'); }
            ...
            function activarReloj(turno) { document.getElementById('card_m').classList.remove('active'); document.getElementById('card_t').classList.remove('active'); document.getElementById('card_'+turno).classList.add('active'); }
                    ...
                    if (outContainer.offsetHeight > 550) {
                        ...
                        let HTML_Con_Corte = `
                            ...
                            <div style="display:flex; justify-content:space-between; align-items:center; width:100%; margin-bottom:10px;">
                                <span style="font-family:'Caveat', cursive; color:var(--celeste-obra); font-size:18px; font-weight:bold;">... Viene del Asiento No ${as_str} del Residente de Obra</span>
                                <span style="font-family:'Caveat', cursive; color:var(--celeste-obra); font-size:18px; font-weight:bold;">${g_fechaAsiento}</span>
                            </div>
                        `;
